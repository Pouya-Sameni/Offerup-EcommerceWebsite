...
@NoArgsConstructor
@Component
public class DutchAuctionAccessor implements AuctionAccessors {
    ...
    public boolean isUserWinner(String auctionId, String userid) {
        try {
                ...
                String url = String.format(API_URL + "dutch/user/winner?auctionId=%s&userId=%s", auctionId, userid);
    ...
    @Override
    public DutchAuction auctionDetails(String auctionId) {
        try {
                String url = String.format(API_URL + "dutch/search/id?auctionId=%s", auctionId);

                ResponseEntity<DutchResponse> response = restTemplate.getForEntity(url, DutchResponse.class);
